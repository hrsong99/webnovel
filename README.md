...
작품에 선택적 `manuscript/glossary.en.json`이 있으면 한국어 회차에서 선별된 무협·고급 어휘의 회차별 첫 등장에 점선 밑줄이 표시됩니다. 단어를 누르면 정적 빌드에 포함된 영어 뜻과 짧은 맥락 설명이 열립니다. 외부 번역 API는 호출하지 않으며 한국어 정본과 영어판·EPUB에는 주석 문법을 섞지 않습니다. 긴 표현을 먼저 일치시켜 겹치는 항목을 안전하게 처리하고, 빌드는 중복·빈 뜻·본문에 없는 항목을 거부합니다.
